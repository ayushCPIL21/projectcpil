...
function CardsItems({ ID, title, desc, freq, isActive, lob, time, expressions,refresh , setRefresh }) {
  const parameter = ID;
  ...
  async function onClickHandle() {
    ...
    try {
      ...
      toast.success(error.response.data.detail[0]);
    }
  ...
  async function onSubmitHandler(e) {
    ...
    const token = localStorage.getItem("token");
    try {
      const response = await axios.put(`/api/process/${ID}/`,{
        name: newTitle,
        path: newPath,
              ...
              <form className="p-4 md:p-5" onSubmit={onSubmitHandler}>
                <div className="grid gap-4 mb-4 grid-cols-1">
                  ...
                  <div className="">
                    ...
                    >
                      <option value="" disabled hidden>{lob}</option>
                      {lobName.map((item) => (
